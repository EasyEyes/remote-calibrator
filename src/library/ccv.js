  ...
  var parallable = function (file, funct) {
    ...
    return function () {
      ...
      if (arguments.length > 1) {
        ...
        for (i = 0; i < arguments.length - 2; i++) params[i] = arguments[i]
      } else {
      ...
      if (async) {
        return function (complete, error) {
          ...
          for (i in scope.shared)
            ...
            /* delete DOM object, if any */ else if (
              scope.shared[i].tagName !== undefined
            )
...
function get_named_arguments(params, names) {
  if (params.length > 1) {
    var new_params = {}
    for (var i = 0; i < names.length; i++) new_params[names[i]] = params[i]
    return new_params
...
var ccv = {
  ...
  detect_objects: parallable(
    'ccv.js',
    function (canvas, cascade, interval, min_neighbors) {
      if (this && this.shared !== undefined) {
        var params = get_named_arguments(arguments, [
          'canvas',
          'cascade',
          'interval',
          'min_neighbors',
        ...
        this.shared.scale_upto = Math.floor(
          Math.log(
            Math.min(
        ...
        for (i = 0; i < this.shared.cascade.stage_classifier.length; i++)
          this.shared.cascade.stage_classifier[
            i
          ].orig_feature = this.shared.cascade.stage_classifier[i].feature
      }
      function pre(worker_num) {
        ...
        for (i = next * 2; i < scale_upto + next * 2; i++) {
          ...
          ret[i * 4 + 1] = {
            ...
            data: pyr[i * 4 + 1]
              .getContext('2d')
              .getImageData(0, 0, pyr[i * 4 + 1].width, pyr[i * 4 + 1].height)
              .data,
          }
          ...
          ret[i * 4 + 2] = {
            ...
            data: pyr[i * 4 + 2]
              .getContext('2d')
              .getImageData(0, 0, pyr[i * 4 + 2].width, pyr[i * 4 + 2].height)
              .data,
          }
          ...
          ret[i * 4 + 3] = {
            ...
            data: pyr[i * 4 + 3]
              .getContext('2d')
              .getImageData(0, 0, pyr[i * 4 + 3].width, pyr[i * 4 + 3].height)
              .data,
          }
        }
        return [ret]
                ...
                for (j = 0; j < cascade.stage_classifier.length; j++) {
                  ...
                  for (k = 0; k < cascade.stage_classifier[j].count; k++) {
                    ...
                    var p,
                      pmin =
                        u8[feature_k.pz[0]][
                          u8o[feature_k.pz[0]] + feature_k.px[0]
                        ]
                    var n,
                      nmax =
                        u8[feature_k.nz[0]][
                          u8o[feature_k.nz[0]] + feature_k.nx[0]
                        ]
                    if (pmin <= nmax) {
                      sum += alpha[k * 2]
                    } else {
...
onmessage = function (event) {
  ...
  var result = parallable.core[data.name].apply(scope, [
    data.input,
    data.id,
    data.worker,
  ])
